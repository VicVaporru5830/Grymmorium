// ======================
// server.js — Grymmorium Backend (tema mágico astral en IA y YouTube)
// ======================
...
// =========================================
// IA (MAGIA / ALQUIMIA) — Router API (MODELO FUNCIONAL)
// =========================================
app.post("/chat", async (req, res) => {
  try {
    ...
    // Prompt especializado (mágico astral)
    const systemPrompt = `
Eres un oráculo arcano con siglos de estudio en magia astral, alquimia y hechicería.
Respondes EXCLUSIVAMENTE sobre temas mágicos (pociones, símbolos, runas, rituales, constelaciones, grimorios).
Usa un tono místico pero claro y didáctico, con pasos, ingredientes y advertencias cuando aplique.
Si te preguntan algo fuera de magia, redirígelo amablemente a un enfoque mágico.
`;
    ...
    const respuesta = resp.data?.choices?.[0]?.message?.content?.trim() || "No pude generar respuesta.";
    res.json({ respuesta });
  } catch (error) {
...
app.get("/youtube", async (_req, res) => {
  try {
    if (!process.env.YOUTUBE_API_KEY) return res.status(500).json({ error: "Falta YOUTUBE_API_KEY" });
    const r = await axios.get("https://www.googleapis.com/youtube/v3/search", {
      params: {
        part: "snippet",
        q: "magia astral alquimia hechicería rituales documentales",
        type: "video",
        maxResults: 6,
...
const uploadVideo = multer({
  ...
  fileFilter: (_req, file, cb) => {
    if (!allowedVideoMimes.includes(file.mimetype)) return cb(new Error("Formato inválido"));
    cb(null, true);
  },
});
app.post("/upload", uploadVideo.single("video"), async (req, res) => {
  ...
  try {
    if (!process.env.S3_BUCKET) return res.status(500).json({ error: "Falta S3_BUCKET" });
